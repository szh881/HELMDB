...
function install_gaussdb()
{
    ...
    if [ "$product_mode"x == "opengauss"x ]; then
        ...
        if [ "$version_mode"x == "release"x ]; then
            ...
            ./configure $shared_opt CFLAGS="-O2 -g3 ${GAUSSDB_EXTRA_FLAGS}" --enable-mot --enable-nvmdb CC=g++ $extra_config_opt >> "$LOG_FILE" 2>&1
        elif [ "$version_mode"x == "memcheck"x ]; then
            ./configure $shared_opt CFLAGS="-O0" --enable-mot --enable-nvmdb --enable-debug --enable-cassert --enable-memory-check CC=g++ $extra_config_opt >> "$LOG_FILE" 2>&1
        elif [ "$version_mode"x == "fiurelease"x ]; then
            ./configure $shared_opt CFLAGS="-O2 -g3 ${GAUSSDB_EXTRA_FLAGS}" --enable-mot --enable-nvmdb --disable-jemalloc CC=g++ $extra_config_opt >> "$LOG_FILE" 2>&1
        elif [ "$version_mode"x == "fiudebug"x ]; then
            ./configure $shared_opt CFLAGS="-O0 ${GAUSSDB_EXTRA_FLAGS}" --enable-mot --enable-nvmdb --enable-debug --enable-cassert --disable-jemalloc CC=g++ $extra_config_opt >> "$LOG_FILE" 2>&1
        else
            ./configure $shared_opt CFLAGS="-O0 ${GAUSSDB_EXTRA_FLAGS}" --enable-mot --enable-nvmdb --enable-debug --enable-cassert CC=g++ $extra_config_opt >> "$LOG_FILE" 2>&1
        fi
