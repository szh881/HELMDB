...
int exprLocation(const Node* expr)
{
    ...
    switch (nodeTag(expr)) {
        ...
        case T_RangeCypher:
            /* use the contained RangeVar's location --- close enough */
            loc = exprLocation((Node*)((const RangeCypher*)expr)->relation);
            break;
        case T_TypeName:
...
bool expression_tree_walker(Node* node, bool (*walker)(), void* context)
{
    ...
    switch (nodeTag(node)) {
        ...
        case T_CypherMatchExpr: {
            CypherMatchExpr* cc = (CypherMatchExpr*)node;
            return true;
        } break;
        case T_CypherStmt: {
            CypherStmt* cc = (CypherStmt*)node;
            return true;
        } break;
...
bool raw_expression_tree_walker(Node* node, bool (*walker)(), void* context)
{
    ...
    switch (nodeTag(node)) {
        ...
        case T_RangeCypher: {
            RangeCypher* rc = (RangeCypher*)node;

            if (p2walker(rc->relation, context)) {
                return true;
            }
        } break;
        case T_TypeName: {
