...
static bool searchModelTypeBySysCache(Oid relid,char type){
    enum SysCacheIdentifier sci;
    HeapTuple tup;
    bool res = false;
    switch (type) {
        case ARRAY_TABLE_MODEL_TYPE:{ // array table
            sci = ARRAYID;
        }break;
        case DOCUMENT_TABLE_MODEL_TYPE:{ // document table
            sci = DOCSID;
        }break;
        case VECTOR_TABLE_MODEL_TYPE:{ // vector table
            sci = VECTORID;
        }break;
        case GRAPH_TABLE_MODEL_TYPE:{ // graph table
            sci = GRAPHOID;
        }break;
        default:
            return res;
    }
    tup = SearchSysCache1(sci, ObjectIdGetDatum(relid));
...
static bool searchModelTypeByHeap(Oid relid,char type){
    Relation mm_catalog_desc;
    SysScanDesc scan_desc;
    ScanKeyData scan_key;
    HeapTuple tup;
    Oid index_oid;
    AttrNumber oid_column_num;
    bool res = false;
    switch (type) {
        case ARRAY_TABLE_MODEL_TYPE:{ // array table
            mm_catalog_desc = heap_open(ArrayRelationId, AccessShareLock);
            index_oid = MmArrayIdIndex;
            oid_column_num = Anum_mm_array_arrid;
        }break;
        case DOCUMENT_TABLE_MODEL_TYPE:{ // document table
            mm_catalog_desc = heap_open(DocumentsRelationId, AccessShareLock);
            index_oid = MmDocsIdIndex;
            oid_column_num = Anum_mm_documents_docsid;
        }break;
        case VECTOR_TABLE_MODEL_TYPE:{ // vector table
            mm_catalog_desc = heap_open(VectorRelationId, AccessShareLock);
            index_oid = MmVectorIdIndex;
            oid_column_num = Anum_mm_vector_vecid;
        }break;
        case GRAPH_TABLE_MODEL_TYPE:{ // graph table
            mm_catalog_desc = heap_open(GraphRelationId, AccessShareLock);
            index_oid = GraphOidIndexId;
            oid_column_num = Anum_mm_graph_graphoid;
        }break;
        default:
            return res;
    }
    ScanKeyInit(&scan_key, oid_column_num, BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(relid));

    scan_desc = systable_beginscan(mm_catalog_desc, index_oid, true, NULL, 1, &scan_key);
    
    tup = systable_getnext(scan_desc);

    if (HeapTupleIsValid(tup)) {
        res = true;
    }
    systable_endscan(scan_desc);
    heap_close(mm_catalog_desc, AccessShareLock);

    return res;
}
/**
 *  check the type of given relid table 
 *  @param relid: oid of table
 *  @param type: expected type of table
 *  @return bool: whether type of table is right
 *  @required: relid is oid of a table
*/
bool checkModelType(Oid relid,char type){
    // if(!u_sess->syscache_cxt.CacheInitialized){
    //     return searchModelTypeByHeap(relid,type);
    // }

    return searchModelTypeBySysCache(relid,type);
}
/* determines the model of table:
* 't': normal   table (default)
* 'g': graph    table
* 'd': document table
* 'v': vector   table
* 'a': array    table
*/
char queryModelType(Oid relid){
    
    char types[] = OTHER_MODEL_TYPES_ARRAY;
    int typesNum = OTHER_MODEL_TYPE_NUMS;

    char resType = NORMAL_TABLE_MODEL_TYPE;
    for(int i = 0;i<typesNum;i++){
        char type = types[i];
        if(checkModelType(relid, type)){
            resType = type;
            break;
        }
    }
    ...
    bool isFirstNode, bool isCreateView, bool isSupportSynonym)
{
    ...
    rte->mm_type = queryModelType(RelationGetRelid(rel));
    ...
    ParseState* pstate, Query* subquery, Alias* alias, bool lateral, bool inFromCl, bool sublinkPullUp)
{
    ...
    //todo: 如果是生成json的子查询标记isJSON为true
    rte->isJSON = false;
    ...
    ParseState* pstate, char* funcname, Node* funcexpr, RangeFunction* rangefunc, bool lateral, bool inFromCl)
{
    ...
    rte->isJSON = strcmp(funcname,"json_array_elements") == 0;
...
/*
 * Add an entry for a cypher query to the pstate's range table (p_rtable).
 *
 * This is just like addRangeTableEntry() except that it makes a cypher RTE.
 */
RangeTblEntry* addRangeTableEntryForCypher(
    ParseState* pstate, Query* cypher_query, Alias* alias,RangeTblEntry* graph_te, bool inFromCl)
{
    RangeTblEntry* rte = graph_te;
    char* refname = NULL;
    Alias* eref = NULL;
    int numaliases;
    int varattno;
    ...
    if (cypher_query == NULL) {
        ereport(
            ERROR, (errmodule(MOD_OPT), errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED), errmsg("cypher query can not be NULL")));
    }

    refname = query_alias->aliasname;
    rte->rtekind = RTE_SUBQUERY;
    rte->subquery = cypher_query;
    rte->alias = query_alias;
    rte->sublink_pull_up = false;
    rte->orientation = REL_ORIENT_UNKNOWN;
    eref = (Alias*)copyObject(query_alias);
    numaliases = list_length(eref->colnames);
    ...
    /* fill in any unspecified alias columns */
    varattno = 0;
    foreach (tlistitem, cypher_query->targetList) {
        TargetEntry* te = (TargetEntry*)lfirst(tlistitem);
        ...
        if (varattno > numaliases) {
            ...
            if (te->resname != NULL &&
                IsQuerySWCBRewrite(cypher_query) &&
                strstr(te->resname, "@")) {
                rte->swSubExist = true;
