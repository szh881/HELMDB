...
static CypherMatchExpr* transformCypherMatchClause(ParseState* pstate, CypherPath* cypher_path);
static void transformCypherFromClause(ParseState* pstate, CypherPath* cypher_path);
static List* transformCypherTargetList(ParseState* pstate, CypherPath* cypher_path);
/*
 * parse_analyze
...
static bool StorageEngineUsedWalker(Node* node, RTEDetectorContext* context)
{
    ...
    if (IsA(node, RangeTblRef)) {
        ...
        if (rte->rtekind == RTE_RELATION) {
            ...
            } else if (rte->relkind == RELKIND_FOREIGN_TABLE && isNVMFromTblOid(rte->relid)) {
                context->isNvmdbTable = true;
...
static Query* transformSelectStmt(ParseState* pstate, SelectStmt* stmt, bool isFirstNode, bool isCreateView)
{
    ...
    /* process the ArraySelect Clause */
    if(stmt->is_array_select == true){
        qry->is_array_select = true;
        qry->arraySelectClause = transformArraySelectClause(stmt->arraySelectClause);
    }
...
/*
 * transformCypherStmt -
 *	  transforms a Cypher Statement
 *
 */
static Query* transformCypherStmt(ParseState* pstate, CypherStmt* stmt){
    Query	  * qry = makeNode(Query);
    CypherPath* cypher_path = (CypherPath *)linitial(stmt->match_pattern);
    Node* qual = NULL;
    qry->commandType = CMD_SELECT;

    /* process the FROM clause */
    transformCypherFromClause(pstate, cypher_path);

    /* transform targetlist */
    qry->targetList = transformCypherTargetList(pstate, cypher_path);

    /* process the Match clause */
    qry->cypher_match = transformCypherMatchClause(pstate, cypher_path);
    qry->is_cypher_query = true;
    /* TODO: transform WHERE clause */
    /* TODO: transform SORT clause */
    /* TODO: transform LIMIT clause */
...
static CypherMatchExpr* transformCypherMatchClause(ParseState* pstate, CypherPath* cypher_path){
    ListCell* match_node = NULL;
    List* match_node_list = NIL;
    List* match_rel_list = NIL;
    CypherMatchExpr* cypher_clause = makeNode(CypherMatchExpr);

    /*
     * process MATCH clause
     * e.g. Match {(p1:publication)-[rel:cites]->(p2:publication)}
     * match_node: (p1:publication), (p2:publication)
     * match_rel: [rel:cites]
     */ 
    foreach (match_node, cypher_path->chain) {
        if(nodeTag(lfirst(match_node))== NodeTag::T_CypherNode){ // node case
            CypherNode* cypher_node = (CypherNode*) lfirst(match_node);
            match_node_list = lappend(match_node_list, cypher_node);
        }else if(nodeTag(lfirst(match_node))== NodeTag::T_CypherRel){ // rel case 
            CypherRel* cypher_rel = (CypherRel*) lfirst(match_node);
            match_rel_list = lappend(match_rel_list, cypher_rel);
        }
    }

    // transform match_list into CypherMatchExpr
    cypher_clause->match_node_list = match_node_list;
    cypher_clause->match_rel_list = match_rel_list;
    return cypher_clause;
}

static void transformCypherFromClause(ParseState* pstate, CypherPath* cypher_path){
    ListCell* match_node = NULL;
    List* from_list = NIL;
    RangeVar* range_var = NULL;

    /*
     * add base graph table from parent pstate's o_rtable into subquery from_list, 
     * later the cypher path generate will use it.
     * e.g. FROM citation_network MATCH xxx
     * => FROM citation_network, xxx;
     */
    Assert(pstate->parentParseState != NULL);
    RangeTblEntry* graph_rte = (RangeTblEntry*)llast(pstate->parentParseState->p_rtable);
    Assert(graph_rte->mm_type == GRAPH_TABLE_MODEL_TYPE);
    range_var = makeRangeVar(NULL,graph_rte->eref->aliasname,-1);
    from_list = lappend(from_list,range_var);
    /*
     * generate from_list
     * e.g. 
     *   MATCH {(p1:publication)-[rel:cites]->(p2:publication)}
     * the above MATCH clause is equivalent to:
     *   FROM publication as p1, cite as rel, publication as p2;
     */ 
    foreach (match_node, cypher_path->chain) {
        if(nodeTag(lfirst(match_node))== NodeTag::T_CypherNode){ // node case
            CypherNode* cypher_node = (CypherNode*) lfirst(match_node);
            CypherName* node_label = (CypherName*) cypher_node->label;
            CypherName* node_name = (CypherName*) cypher_node->variable;
            // transform node into RangeVar
            range_var = makeRangeVar(NULL,node_label->name,-1);
            range_var->alias = makeNode(Alias);
            range_var->alias->aliasname = node_name->name;
        }else if(nodeTag(lfirst(match_node))== NodeTag::T_CypherRel){ // rel case 
            CypherRel* cypher_rel = (CypherRel*) lfirst(match_node);
            CypherName* rel_label = (CypherName*) linitial(cypher_rel->types);
            CypherName* rel_name = (CypherName*) cypher_rel->variable;
            // transform rel into RangeVar
            range_var = makeRangeVar(NULL,rel_label->name,-1);
            range_var->alias = makeNode(Alias);
            range_var->alias->aliasname = rel_name->name;
        }
        from_list = lappend(from_list,range_var);
    }

    // transform from_list
    transformFromClause(pstate,from_list,false,false);
    return;
}
...
static List* transformCypherTargetList(ParseState* pstate, CypherPath* cypher_path){
     ...
     */ 
    foreach (match_node, cypher_path->chain) {
        // initialize a ResTarget
        res_target = makeNode(ResTarget);
        res_target->name = NULL; // alias
        ...
        if(nodeTag(lfirst(match_node))== NodeTag::T_CypherNode){ // node case
            CypherNode* cypher_node = (CypherNode*) lfirst(match_node);
            CypherName* node_name = (CypherName*) cypher_node->variable;
            // transform node into ColumnRef
            val = (Node *)makeStarColumnRef(node_name->name, -1);

        }else if(nodeTag(lfirst(match_node))== NodeTag::T_CypherRel){ // rel case 
            CypherRel* cypher_rel = (CypherRel*) lfirst(match_node);
            CypherName* rel_name = (CypherName*) cypher_rel->variable;
            // transform rel into ColumnRef
            val = (Node *)makeStarColumnRef(rel_name->name, -1);
        }
        res_target->val = val;
        target_list = lappend(target_list, res_target);
    }
    // transform target_list
    List* target_entry_list = transformTargetList(pstate, target_list, EXPR_KIND_SELECT_TARGET);
    ListCell* target_entry_cell = NULL;
    foreach (target_entry_cell, target_entry_list) {
        TargetEntry* te = (TargetEntry*) lfirst(target_entry_cell);
        RangeTblEntry* rte = rt_fetch(((Var*)te->expr)->varno,pstate->p_rtable);
        /* 
         * transform col into label.col 
         * e.g. properties => p1.properties
         */
        StringInfo si = makeStringInfo();
        appendStringInfo(si, "%s.%s", rte->alias->aliasname, te->resname);
        te->resname = si->data;
    }
    return target_entry_list;
}
