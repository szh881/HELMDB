...
/* For Graph Query */
static RangeTblEntry* transformRangeCypher(ParseState* pstate, RangeCypher* rc);
...

{
    ...
    errmsg("TABLESAMPLE clause can only be applied to tables, materialized views and key-preserved join views."),
        ...
        *top_rti = j->rtindex;
        ...
        return (Node*)j;
    } else if (IsA(n, RangeCypher)) {
        /* sub-cypher is like a plain relation */
        RangeTblRef* rtr = NULL;
        RangeTblEntry* rte = NULL;

        /* Transform Cypher details and attach to the RTE */
        rte = transformRangeCypher(pstate, (RangeCypher*)n);
...
static RangeTblEntry* transformRangeCypher(ParseState* pstate, RangeCypher* rc){
    ...
    if(unlikely(rc->relation == NULL)){
        ereport(
                ERROR, (errmodule(MOD_OPT), errcode(ERRCODE_UNEXPECTED_NULL_VALUE), errmsg("rel should not be NULL")));
    }

    /* We only support this on plain graph */
    if(!IsA(rc->relation, RangeVar)){
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("Cypher clause can only be applied to tables."),
                    parser_errposition(pstate, exprLocation(rc->relation))));   
    }

    if (rte == NULL) {
        rte = transformTableEntry(pstate, (RangeVar *)(rc->relation), true, false);
    }
    if(rc->alias == NULL){
        rc->alias = makeAlias(((RangeVar *)rc->relation)->relname, NIL);
    } else{
        rc->alias->aliasname = pstrdup(((RangeVar *)rc->relation)->relname);
    }

    if (rte->mm_type != GRAPH_TABLE_MODEL_TYPE) {
        ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                (errmsg("Cypher clause only support relation of graph table"))));
    }
    
    /*
     * Analyze and transform the subquery.
     */
    query = parse_sub_analyze(rc->cypher_query, pstate, NULL, isLockedRefname(pstate, rc->alias->aliasname), true);
    /*
     * OK, build an RTE for the cypher query.
     */
    rte = addRangeTableEntryForCypher(pstate, query, rc->alias, rte, true);
    
    return rte;
}


// 把arraySelectClause复制一份返回, 赋值给Query->arraySelectClause
// TODO: 加上语义检查，当维度数量/属性数量和原数组不符时抛出异常
Node* transformArraySelectClause(Node* arraySelectClause){
    ArraySelectClause *original = (ArraySelectClause*)arraySelectClause;

    if (original == NULL) return NULL;

    ArraySelectClause* result = (ArraySelectClause*) palloc(sizeof(ArraySelectClause));
    if (result == NULL) return NULL;

    // 复制基本和标量类型
    result->type = original->type;
    result->array_name = pstrdup(original->array_name);
    result->dims_num = original->dims_num;
    result->is_point_query = original->is_point_query;
    result->attrs_num = original->attrs_num;
    result->is_all_attrs = original->is_all_attrs;
    if(original->lower_bounds == NULL || 
        original->upper_bounds == NULL ||
        original->lower_bounds_valid == NULL ||
        original->lower_bounds_valid == NULL)
    {
        elog(ERROR, "array dimension infomation shouldn't be NULL\n");
    }
    // 复制维度相关数组
    result->lower_bounds = (int*) palloc(original->dims_num * sizeof(int));
    result->upper_bounds = (int*) palloc(original->dims_num * sizeof(int));
    result->lower_bounds_valid = (bool*) palloc(original->dims_num * sizeof(bool));
    result->upper_bounds_valid = (bool*) palloc(original->dims_num * sizeof(bool));
    for(int i = 0; i < original->dims_num; i++){
        //上下界检查
        if(original->lower_bounds[i]>original->upper_bounds[i])
            elog(ERROR, "invalid bounds: lower bound %d is bigger than upper bound %d\n",original->lower_bounds[i],original->upper_bounds[i]);
        result->lower_bounds[i] = original->lower_bounds[i];
        result->upper_bounds[i] = original->upper_bounds[i];
        result->lower_bounds_valid[i] = original->lower_bounds_valid[i];
        result->upper_bounds_valid[i] = original->upper_bounds_valid[i];
    }
    ...
    if (original->attrs) {
        ...
        foreach(cell, original->attrs) {
            ...
            result->attrs = lappend(result->attrs, strdup(data));
        }
    } else {
        result->attrs = NULL;
    }
