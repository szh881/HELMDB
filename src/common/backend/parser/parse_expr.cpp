...
static Node *transformJSONArrayExpr(ParseState *pstate, A_JSONArrayExpr *a);
static Node *transformJSONObjectExpr(ParseState *pstate, A_JSONObjectExpr *a);

Node *processDocumentIndirection(ParseState *pstate, A_Indirection *ind);
...
Node *transformExprRecurse(ParseState *pstate, Node *expr)
{
    ...
    switch (nodeTag(expr)) {
        ...
        case T_A_JSONArrayExpr:
            result = transformJSONArrayExpr(pstate, (A_JSONArrayExpr*)expr);
            break;

        case T_A_JSONObjectExpr:
            result = transformJSONObjectExpr(pstate, (A_JSONObjectExpr*)expr);
            break;
...
static Node* transformIndirection(ParseState* pstate, A_Indirection* ind)
{
    Node* node = processDocumentIndirection(pstate,ind);
    if(node!=NULL){
...
static Node* makeConstString(Value* value,int location){
 ...
 * 重写文档模型语法树
 *
 * A.a.b.c => ((A.a).b).c 或 a.b.c => ((a).b).c
 */
static Node* rewriteParserTree1(ParseState* pstate, ColumnRef* cref, int reserve){
    Node *ret = (Node*)cref;

    ListCell *newhead = list_nth_cell(cref->fields,reserve-1)->next;

    cref->fields = list_truncate(cref->fields, reserve);
    
    for (ListCell* p = newhead; p!=NULL; p=p->next){
        A_Indirection *n = makeNode(A_Indirection);
		n->arg = ret;
	    n->indirection = list_make1(lfirst(p));
        ret = (Node *)n;
    }
 ...
 * A.a.b.c => (A.a)->'b'->'c' 或 a.b.c => a->'b'->'c'
 */
static Node* rewriteParserTree2(ParseState* pstate, ColumnRef* cref, int reserve){
    ...
    for (ListCell* p = newhead; p!=NULL; p = p->next){
        ret = (Node*)makeSimpleA_Expr(AEXPR_OP,"->",ret,makeConstString(lfirst_node(Value,p),cref->location),cref->location);
    }
 ...
 * 重写文档模型语法树
 *
 * A.b.c => (A.document)->'b'->'c'
 */
static Node* rewriteParserTree3(ParseState* pstate, ColumnRef* cref){

    Node *ret = (Node*)cref;

    ListCell *newhead = cref->fields->head->next;

    cref->fields = list_truncate(cref->fields, 1);

    cref->fields = lappend(cref->fields,makeString(FIXED_DOCUMENT_COLNAME));
 ...
 * b.c => (document)->'b'->'c'
 */
static Node* rewriteParserTree4(ParseState* pstate, ColumnRef* cref){

    Node *ret = (Node*)cref;

    ListCell *newhead = cref->fields->head;

    cref->fields = list_make1(makeString(FIXED_DOCUMENT_COLNAME));
...
static bool checkIsDocument(RangeTblEntry* rte){
    return checkModelType(rte->relid,DOCUMENT_TABLE_MODEL_TYPE);
}

Node* processDocumentColumnRef(ParseState* pstate, ColumnRef* cref){
    Node* node = NULL;
    char* nspname = NULL;
    char* relname = NULL;
    char* colname = NULL;
    ...
    if(rte != NULL && checkIsDocument(rte)){ //第一个字段为文档名
        if(fields_len == 1){ //等同于 A.*
            lappend(cref->fields,(Node *) makeNode(A_Star));
            return NULL;
        } else if (fields_len >= 2) {  // 检查第二个字段是否为文档的静态字段
            Node* field2 = (Node*)lsecond(cref->fields);
            AssertEreport(IsA(field2, String), MOD_OPT, "");
            colname = strVal(field2);
            node = scanRTEForColumn(pstate, rte, colname, cref->location);
            if(node != NULL){ //访问的是静态字段

                if(fields_len == 2){
                    return NULL;
                }

                if(strcmp(colname,FIXED_DOCUMENT_COLNAME) != 0){ 
                  //重写静态字段的语法解析树，使其变成 (((A.a).b).c)...
                  return rewriteParserTree1(pstate, cref,2);
                }else {
                  //重写动态字段的语法解析树，使其变成 (A.document)->"b"->"c"
                  return rewriteParserTree2(pstate, cref,2);
                }

            }else{  //访问的是动态字段
              //重写动态字段的语法解析树，使其变成 (A.document)->"b"->"c"
              return rewriteParserTree3(pstate, cref);
            }
        }
    }else if(pstate->p_rtable != NULL && pstate->p_rtable->length == 1 && 
            checkIsDocument((RangeTblEntry*)(lfirst(list_head(pstate->p_rtable)))) ){ 
        //第一字段为文档的列名，todo: 判断当前from子句中只有一个值，且为文档
        char* colname = relname;
        ...
        if(node != NULL){  //访问的是静态字段
            ...
            if(strcmp(colname,FIXED_DOCUMENT_COLNAME) != 0){ 
                //重写静态字段的语法解析树，使其变成 (((a).b).c)...
                return rewriteParserTree1(pstate, cref,1);
            }else{
                //重写动态字段的语法解析树，使其变成 (document)->"b"->"c"
                return rewriteParserTree2(pstate, cref,1);
            }
        }else{ //访问的是动态字段
             //重写动态字段的语法解析树，使其变成 (document)->"b"->"c"
             if(strcmp(colname,FIXED_DOCUMENT_COLNAME) == 0){
             ...
             return rewriteParserTree4(pstate, cref);
        }
    }else if(rte != NULL && rte->isJSON){
        if(fields_len == 1){
            return NULL;
        } 
        //重写动态字段的语法解析树，使其变成 (a)->"b"->"c"
        return rewriteParserTree2(pstate, cref,1);
    }
...
Node* processDocumentIndirection(ParseState* pstate, A_Indirection* ind){
    ...
    Node* node = processDocumentColumnRef(pstate,(ColumnRef*)(ind->arg));
    if(node == NULL){
        return NULL;
    }

    Node *ret = node;
    int location = exprLocation(ret);
    ...
    if(IsA(node,A_Expr)){
        ...
        foreach (i, ind->indirection) {
            Node* n = (Node*)lfirst(i);
            if (IsA(n, A_Indices)) {
                A_Indices* indice = (A_Indices*)n;
                if(indice->lidx != NULL){
                    ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("this type of slicing is not supported by document model"),
                    parser_errposition(pstate, location)));
                }
                ret = (Node*)makeSimpleA_Expr(AEXPR_OP,"->",ret,indice->uidx,-1);

            }else if(IsA(n, String)){
                ret = (Node*)makeSimpleA_Expr(AEXPR_OP,"->",ret,makeConstString((Value*)n,-1),-1);
            }else{
                ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    errmsg("this type of slicing is not supported by document model"),
    ...
    }else if(IsA(node,A_Indirection)){
        ListCell* i = NULL;
        foreach (i, ind->indirection) {
            Node* n = (Node*)lfirst(i);
            if (IsA(n, A_Indices)) {
                ...
                A_Indirection *tmp = makeNode(A_Indirection);
		        tmp->arg = ret;
	            tmp->indirection = list_make1(n);
                ret = (Node *)tmp;
...
Node* transformColumnRef(ParseState* pstate, ColumnRef* cref)
{
    ...
    Node *ret = processDocumentColumnRef(pstate, cref);
    if(ret!=NULL){
        return transformExprRecurse(pstate, ret);
    }
...
static Node* transformJSONArrayExpr(ParseState* pstate, A_JSONArrayExpr* a)
{
    ...
    n->funcname = list_make1(makeString("json_build_array"));
    n->args = a->elements;
    n->agg_order = NIL;
	n->agg_star = FALSE;
...
{
    FuncCall *n = makeNode(FuncCall);
    n->funcname = list_make1(makeString("json_build_object"));
    n->agg_order = NIL;
	n->agg_star = FALSE;
	n->agg_distinct = FALSE;
	n->func_variadic = FALSE;
    ...
    foreach (element, a->elements) {
        ...
        AssertEreport(IsA(e,JSONObjectPair), MOD_OPT, "");
        JSONObjectPair* p = (JSONObjectPair*)e;

        newelems = lappend(newelems, makeConstString(p->key,p->location));
        newelems = lappend(newelems,p->value);
    }
