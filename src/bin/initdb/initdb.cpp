...
#ifdef ENABLE_NVMDB
static void load_nvm_fdw(void)
{
    int nRet = 0;
    PG_CMD_DECL;

    fputs(_("loading foreign-data wrapper for NVM access ... "), stdout);
    (void)fflush(stdout);
    ...
    PG_CMD_PUTS("CREATE EXTENSION nvm_fdw;\n");

    PG_CMD_PUTS("CREATE SERVER nvm_server FOREIGN DATA WRAPPER nvm_fdw;\n");
