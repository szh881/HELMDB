...
void SetCurrentTransactionStorageEngine(StorageEngineType storageEngineType)
{
    ...
    } else if (storageEngineType == SE_TYPE_NVMDB &&
               (CurrentTransactionState->storageEngineType == SE_TYPE_UNSPECIFIED || IsNVMDBEngineUsed())) {
        CurrentTransactionState->storageEngineType = SE_TYPE_NVMDB;
    }
}
...
#ifdef ENABLE_NVMDB
/*
 * Check if we are using NVMDB storage engine in current transaction.
 */
bool IsNVMDBEngineUsed()
{
    return (CurrentTransactionState->storageEngineType == SE_TYPE_NVMDB);
}
#endif
