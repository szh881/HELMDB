...
CreateElementPages(HnswBuildState * buildstate)
{
	...
	ntup = (HnswNeighborTuple)palloc0(maxSize);
...
CreateNeighborPages(HnswBuildState * buildstate)
{
	...
	/* Allocate once */
	ntup = (HnswNeighborTuple)palloc0(BLCKSZ);
	...
	foreach(lc, buildstate->elements)
	{
		...
		if (!page_index_tuple_overwrite(page, e->neighborOffno, (Item) ntup, ntupSize))
			elog(ERROR, "failed to add index item to \"%s\"", RelationGetRelationName(index));
...
FreeElements(HnswBuildState * buildstate)
{
	...
	foreach(lc, buildstate->elements)
		HnswFreeElement((HnswElement)lfirst(lc));
			  ...
			  bool *isnull, bool tupleIsAlive, void *state)
{
	...
	element->vec = (Vector*)palloc(VECTOR_SIZE(buildstate->dimensions));
...
HnswGetMaxInMemoryElements(int m, double ml, int dimensions)
{
	...
	return (u_sess->attr.attr_memory.maintenance_work_mem * 1024L) / elementSize;
}
...
BuildGraph(HnswBuildState * buildstate, ForkNumber forkNum)
{
	...
	buildstate->reltuples = IndexBuildHeapScan(buildstate->heap, buildstate->index, buildstate->indexInfo,
											   true, (IndexBuildCallback)BuildCallback, (void *) buildstate, NULL);
#endif
}
