		...
		for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
		{
			...
			if (ItemPointerIsValid(&etup->heaptids[0]))
			{
				...
				if (itemUpdated)
				{
					...
					if (!page_index_tuple_overwrite(page, offno, (Item) etup, etupSize))
...
MarkDeleted(HnswVacuumState * vacuumstate)
{
	...
	while (BlockNumberIsValid(blkno))
	{
		...
		for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
		{
			...
			if (!page_index_tuple_overwrite(npage, neighborOffno, (Item) ntup, ntupSize))
				elog(ERROR, "failed to add index item to \"%s\"", RelationGetRelationName(index));
...
InitVacuumState(HnswVacuumState * vacuumstate, IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	vacuumstate->ntup = (HnswNeighborTuple)palloc0(BLCKSZ);
...
Datum hnswbulkdelete(PG_FUNCTION_ARGS)
{
  ...
  IndexBulkDeleteCallback callback = (IndexBulkDeleteCallback)PG_GETARG_POINTER(2);
	...
	PG_RETURN_POINTER(vacuumstate.stats);
}
