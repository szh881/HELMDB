...
HnswInitNeighbors(HnswElement element, int m)
{
	...
	element->neighbors = (HnswNeighborArray *)palloc(sizeof(HnswNeighborArray) * (level + 1));
	...
	for (int lc = 0; lc <= level; lc++)
	{
		...
		a->length = 0;
		a->items = (HnswCandidate *)palloc(sizeof(HnswCandidate) * lm);
	}
}
...
{
	ItemPointer copy = (ItemPointer)palloc(sizeof(ItemPointerData));
...
HnswLoadElementFromTuple(HnswElement element, HnswElementTuple etup, bool loadHeaptids, bool loadVec)
{
	...
	if (loadVec)
	{
		element->vec = (Vector *)palloc(VECTOR_SIZE(etup->vec.dim));
