...
PlanState* ExecInitNodeByType(Plan* node, EState* estate, int eflags)
{
    switch (nodeTag(node)) {
        ...
        case T_ArrayScan:
            return (PlanState*)ExecInitArrayScan((ArrayScan*)node, estate, eflags);
        case T_DocumentScan:
            return (PlanState*)ExecInitDocumentScan((DocumentScan*)node, estate, eflags);
        case T_VectorScan:
            return (PlanState*)ExecInitVectorScan((VectorScan*)node, estate, eflags);
        case T_GraphScan:
            return (PlanState*)ExecInitGraphScan((GraphScan*)node, estate, eflags);
        default:
            ereport(ERROR,
...
static void ExecEndNodeByType(PlanState* node)
{
    ...
    switch (nodeTag(node)) {
        ...
        case T_TrainModelState:
            ...
            break;
        case T_ArrayScanState:
            ExecEndArrayScan((ArrayScanState*)node);
        case T_DocumentScanState:
            ExecEndDocumentScan((DocumentScanState*)node);
        case T_VectorScanState:
            ExecEndVectorScan((VectorScanState*)node);
        case T_GraphScan:
