...
#include "catalog/mm_documents.h"
#include "catalog/mm_graph.h"
#include "catalog/mm_label.h"
#include "catalog/mm_array.h"
#include "catalog/mm_vector.h"
                   ...
                   bool isCTAS)
{
    ...
    PG_TRY();
    {
        ...
        case T_AlterRlsPolicyStmt: /* ALTER ROW LEVEL SECURITY POLICY */
            ...
            case T_CreateArrayStmt: {
                CreateArrayStmt *fake_stmt = (CreateArrayStmt*)parse_tree;
                Constraint  *constraint_pk = makeNode(Constraint);
                ListCell    *lc;
                List        *pk_cols = NIL;
                Relation    mm_array;
                bool		nulls[Natts_mm_array];
                Datum		values[Natts_mm_array];
                HeapTuple	tup;
                Oid         array_oid = InvalidOid;
                Relation    new_array;
                int         dim_num = list_length(fake_stmt->dim_list);
                int         attr_num = list_length(fake_stmt->stmt->tableElts);

                /*
                 * Transform the CreateDocumentsStmt to a CreateStmt:
                 * append an ID column and a JSONB column to column list 
                 * in the Create Stmt.
                 */              
                foreach(lc, fake_stmt->dim_list)    
                {
                    ColumnDef   *coldef = (ColumnDef *)lfirst(lc);
                    IndexElem   *ie = makeNode(IndexElem);

                    ie->name = coldef->colname;
					ie->expr = NULL;
                    ...
                    pk_cols = lappend(pk_cols, ie);
                }       

                constraint_pk->contype = CONSTR_PRIMARY;
                constraint_pk->location = -1;
                constraint_pk->keys = pk_cols;
                constraint_pk->including = NIL;
                constraint_pk->options = NIL;
                constraint_pk->indexname = NULL;
                constraint_pk->indexspace = NULL;
                constraint_pk->deferrable = false;
                constraint_pk->initdeferred = false;
                constraint_pk->inforConstraint = makeNode(InformationalConstraint);
                constraint_pk->inforConstraint->nonforced = false;
                constraint_pk->inforConstraint->enableOpt = false;
                
                fake_stmt->stmt->tableElts = lappend(fake_stmt->stmt->tableElts, constraint_pk);

                fake_stmt->stmt->tableElts = list_concat(fake_stmt->dim_list, fake_stmt->stmt->tableElts); 

#ifdef PGXC
                CreateCommand(fake_stmt->stmt, query_string, params, is_top_level, sent_to_remote, isCTAS);
#else
                CreateCommand(fake_stmt->stmt, query_string, params, is_top_level, isCTAS);
#endif
                /*
                 * Insert meta-data for the newly created array object.
                 */
                mm_array = relation_open(ArrayRelationId, RowExclusiveLock);
                memset(values, 0, sizeof(values));
                memset(nulls, false, sizeof(nulls));

                array_oid = RangeVarGetRelid(fake_stmt->stmt->relation, AccessShareLock, false);
                new_array = RelationIdGetRelation(array_oid);

                values[Anum_mm_array_arrname - 1] =
                    DirectFunctionCall1(namein, CStringGetDatum(fake_stmt->stmt->relation->relname));
                values[Anum_mm_array_arrid - 1] = ObjectIdGetDatum(array_oid);
                values[Anum_mm_array_arrnsp - 1] = ObjectIdGetDatum(RelationGetNamespace(new_array));
                values[Anum_mm_array_arrdimnum - 1] = Int32GetDatum(dim_num);
                values[Anum_mm_array_arrattrnum - 1] = Int32GetDatum(attr_num);
                values[Anum_mm_array_arrasmatrix - 1] = BoolGetDatum(false);

                RelationClose(new_array);

                tup = heap_form_tuple(RelationGetDescr(mm_array), values, nulls);

                tup->t_tableOid = array_oid;

                CatalogTupleInsert(mm_array, tup);
                heap_freetuple(tup);

                relation_close(mm_array, RowExclusiveLock);
            ...
            case T_CreateDocumentsStmt: {
                CreateDocumentsStmt *doc_stmt = (CreateDocumentsStmt*)parse_tree;
                CreateStmt          *c_stmt = makeNode(CreateStmt);
                ColumnDef           *col_id = makeNode(ColumnDef);
                ColumnDef           *col_doc = makeNode(ColumnDef);
                List                *templist = NULL;
                ListCell            *lc = NULL;
                Relation            mm_docs;
                bool		        nulls[Natts_mm_documents];
                Datum		        values[Natts_mm_documents];
                HeapTuple	        tup;
                Oid                 docs_oid = InvalidOid;
                Relation            new_docs;

                c_stmt->relation = doc_stmt->relation;
                c_stmt->tableElts = doc_stmt->tableElts;
                c_stmt->inhRelations = NIL;
                c_stmt->constraints = NIL;
                c_stmt->options = NIL;
                c_stmt->oncommit = ONCOMMIT_NOOP;
                c_stmt->row_compress = REL_CMPRS_PAGE_PLAIN;
                c_stmt->tablespacename = NULL;
                c_stmt->if_not_exists = true;
#ifdef PGXC
                c_stmt->distributeby = NULL;
                c_stmt->subcluster = NULL;
#endif
                c_stmt->tableOptions = NIL;
                c_stmt->partTableState = NULL;
                c_stmt->internalData = NULL;
                c_stmt->autoIncStart = NULL;
                c_stmt->charset = PG_INVALID_ENCODING;
                /*
                 * Transform the CreateDocumentsStmt to a CreateStmt:
                 * append an ID column and a JSONB column to column list 
                 * in the Create Stmt.
                 */
                foreach(lc, c_stmt->tableElts)
                {
                    ColumnDef *tempcoldef = (ColumnDef *)lfirst(lc);

                    if ((pg_strcasecmp(tempcoldef->colname, "ID") == 0) && (pg_strcasecmp(tempcoldef->colname, FIXED_DOCUMENT_COLNAME) == 0))
                        elog(ERROR, "The name of descriptive attributes in Documents can not be 'id' or 'doc'.");
                }

                col_id->colname = "id";
                col_id->typname = makeTypeName("serial");
                col_id->typname->charset = PG_INVALID_ENCODING;
				col_id->kvtype = ATT_KV_UNDEFINED;
                col_id->inhcount = 0;
                col_id->is_local = true;
                col_id->is_not_null = false;
                col_id->is_from_type = false;
                col_id->storage = 0;
                col_id->cmprs_mode = ATT_CMPR_UNDEFINED;
                col_id->raw_default = NULL;
                col_id->update_default = NULL;
                col_id->cooked_default = NULL;
                col_id->collOid = InvalidOid;
                col_id->fdwoptions = NIL;
                col_id->constraints = NIL;
                col_id->collClause = NULL;
                col_id->clientLogicColumnRef = NULL;
				col_id->columnOptions = NIL;

                templist = list_make1(col_id);
                c_stmt->tableElts = list_concat(templist, c_stmt->tableElts);


                col_doc->colname = FIXED_DOCUMENT_COLNAME;
                col_doc->typname = makeTypeName("jsonb");
                col_doc->typname->charset = PG_INVALID_ENCODING;
				col_doc->kvtype = ATT_KV_UNDEFINED;
                col_doc->inhcount = 0;
                col_doc->is_local = true;
                col_doc->is_not_null = false;
                col_doc->is_from_type = false;
                col_doc->storage = 0;
                col_doc->cmprs_mode = ATT_CMPR_UNDEFINED;
                col_doc->raw_default = NULL;
                col_doc->update_default = NULL;
                col_doc->cooked_default = NULL;
                col_doc->collOid = InvalidOid;
                col_doc->fdwoptions = NIL;
                col_doc->constraints = NIL;
                col_doc->collClause = NULL;
                col_doc->clientLogicColumnRef = NULL;
				col_doc->columnOptions = NIL;

                templist = list_make1(col_doc);
                c_stmt->tableElts = list_concat(c_stmt->tableElts, templist);

#ifdef PGXC
                CreateCommand(c_stmt, query_string, params, is_top_level, sent_to_remote, isCTAS);
#else
                CreateCommand(c_stmt, query_string, params, is_top_level, isCTAS);
#endif
                /*
                 * Insert meta-data for the newly created documents object.
                 */
                mm_docs = relation_open(DocumentsRelationId, RowExclusiveLock);
                memset(values, 0, sizeof(values));
                memset(nulls, false, sizeof(nulls));
                
                docs_oid = RangeVarGetRelid(c_stmt->relation, AccessShareLock, false);
                new_docs = RelationIdGetRelation(docs_oid);

                values[Anum_mm_documents_docsname - 1] =
                    DirectFunctionCall1(namein, CStringGetDatum(c_stmt->relation->relname));
                values[Anum_mm_documents_docsid - 1] = ObjectIdGetDatum(docs_oid);
                values[Anum_mm_documents_docsnsp - 1] = ObjectIdGetDatum(RelationGetNamespace(new_docs));

                RelationClose(new_docs);

                tup = heap_form_tuple(RelationGetDescr(mm_docs), values, nulls);

                tup->t_tableOid = docs_oid;

                CatalogTupleInsert(mm_docs, tup);
                heap_freetuple(tup);

                relation_close(mm_docs, RowExclusiveLock);
            ...
            case T_CreateGraphStmt: {
                CreateGraphStmt     *graph_stmt = (CreateGraphStmt*)parse_tree;
                CreateStmt          *c_graphstmt = makeNode(CreateStmt);
                // Constraint          *graph_constraint_pk = makeNode(Constraint);
                // List                *graph_pk_cols = NIL;
                //IndexElem           *graph_ie = makeNode(IndexElem);
                ColumnDef           *col_id = makeNode(ColumnDef);
                ColumnDef           *col_labelname = makeNode(ColumnDef);
                ColumnDef           *col_labeltype = makeNode(ColumnDef);
                List                *templist = NULL;
                ListCell            *lc = NULL;
                Relation            mm_graph; /* catalog */
                Relation            new_graph; /* insert new graph relation */
                Oid                 graph_oid = InvalidOid;
                bool                graph_nulls[Natts_mm_graph];
                Datum               graph_values[Natts_mm_graph];
                HeapTuple           tup;
                /*
                 * Transform the CreateGraphStmt to a CreateStmt:
                 * append an ID column and a labelname column to column list 
                 * in the Create Stmt.
                 */
                col_id->colname = "id";
                col_id->typname = makeTypeName("serial");
                ...
                col_labelname->colname = "labelname";
                col_labelname->typname = makeTypeName("text");
                col_labelname->typname->charset = PG_INVALID_ENCODING;
				col_labelname->kvtype = ATT_KV_UNDEFINED;
                col_labelname->inhcount = 0;
                col_labelname->is_local = true;
                col_labelname->is_not_null = false;
                col_labelname->is_from_type = false;
                col_labelname->storage = 0;
                col_labelname->cmprs_mode = ATT_CMPR_UNDEFINED;
                col_labelname->raw_default = NULL;
                col_labelname->update_default = NULL;
                col_labelname->cooked_default = NULL;
                col_labelname->collOid = InvalidOid;
                col_labelname->fdwoptions = NIL;
                col_labelname->constraints = NIL;
                col_labelname->collClause = NULL;
                col_labelname->clientLogicColumnRef = NULL;
				col_labelname->columnOptions = NIL;

                col_labeltype->colname = "labeltype";
                col_labeltype->typname = makeTypeName("char");
                col_labeltype->typname->charset = PG_INVALID_ENCODING;
				col_labeltype->kvtype = ATT_KV_UNDEFINED;
                col_labeltype->inhcount = 0;
                col_labeltype->is_local = true;
                col_labeltype->is_not_null = false;
                col_labeltype->is_from_type = false;
                col_labeltype->storage = 0;
                col_labeltype->cmprs_mode = ATT_CMPR_UNDEFINED;
                col_labeltype->raw_default = NULL;
                col_labeltype->update_default = NULL;
                col_labeltype->cooked_default = NULL;
                col_labeltype->collOid = InvalidOid;
                col_labeltype->fdwoptions = NIL;
                col_labeltype->constraints = NIL;
                col_labeltype->collClause = NULL;
                col_labeltype->clientLogicColumnRef = NULL;
				col_labeltype->columnOptions = NIL;

                // templist = list_make3(col_id, col_labelname, col_labeltype);
                templist = list_make2(col_id, col_labelname); 
                // graph_ie->name = col_id->colname;
                // graph_ie->expr = NULL;
                // graph_ie->indexcolname = NULL;
                // graph_ie->collation = NIL;
                // graph_ie->opclass = NIL;
                // graph_ie->ordering = SORTBY_DEFAULT;
                // graph_ie->nulls_ordering = SORTBY_NULLS_DEFAULT;

                // graph_pk_cols = lappend(graph_pk_cols, graph_ie);

                // graph_constraint_pk->contype = CONSTR_PRIMARY;
                // graph_constraint_pk->location = -1;
                // graph_constraint_pk->keys = graph_pk_cols;
                // graph_constraint_pk->including = NIL;
                // graph_constraint_pk->options = NIL;
                // graph_constraint_pk->indexname = NULL;
                // graph_constraint_pk->indexspace = NULL;
                // graph_constraint_pk->deferrable = false;
                // graph_constraint_pk->initdeferred = false;
                // graph_constraint_pk->inforConstraint = makeNode(InformationalConstraint);
                // graph_constraint_pk->inforConstraint->nonforced = false;
                // graph_constraint_pk->inforConstraint->enableOpt = false;

                // templist = lappend(templist, graph_constraint_pk); 

                c_graphstmt->relation = graph_stmt->relation;
                c_graphstmt->tableElts = templist;
                c_graphstmt->inhRelations = NIL;
                c_graphstmt->constraints = NIL;
                c_graphstmt->options = NIL;
                c_graphstmt->oncommit = ONCOMMIT_NOOP;
                c_graphstmt->row_compress = REL_CMPRS_PAGE_PLAIN;
                c_graphstmt->tablespacename = NULL;
                c_graphstmt->if_not_exists = true;
#ifdef PGXC
                c_graphstmt->distributeby = NULL;
                c_graphstmt->subcluster = NULL;
#endif
                c_graphstmt->tableOptions = NIL;
                c_graphstmt->partTableState = NULL;
                c_graphstmt->internalData = NULL;
                c_graphstmt->autoIncStart = NULL;
                c_graphstmt->charset = PG_INVALID_ENCODING;

#ifdef PGXC
                CreateCommand(c_graphstmt, query_string, params, is_top_level, sent_to_remote, isCTAS);
#else
                CreateCommand(c_graphstmt, query_string, params, is_top_level, isCTAS);
#endif

                /*
                 * Insert meta-data for the newly created graph object.
                 */
                mm_graph = relation_open(GraphRelationId, RowExclusiveLock);
                memset(graph_values, 0, sizeof(graph_values));
                memset(graph_nulls, false, sizeof(graph_nulls));

                graph_oid = RangeVarGetRelid(graph_stmt->relation, AccessShareLock, false);
                new_graph = RelationIdGetRelation(graph_oid);

                graph_values[Anum_mm_graph_graphname - 1] = DirectFunctionCall1(namein, CStringGetDatum(graph_stmt->relation->relname));
                graph_values[Anum_mm_graph_graphoid - 1] = ObjectIdGetDatum(graph_oid);
                graph_values[Anum_mm_graph_nspid - 1] = ObjectIdGetDatum(RelationGetNamespace(new_graph));

                RelationClose(new_graph);

                tup = heap_form_tuple(RelationGetDescr(mm_graph), graph_values, graph_nulls);

                tup->t_tableOid = graph_oid;

                CatalogTupleInsert(mm_graph, tup);

                heap_freetuple(tup);

                relation_close(mm_graph, RowExclusiveLock);

                /*
                 * Create label relation.
                */
                foreach(lc, graph_stmt->labelList){
                    GraphLabel  *label = (GraphLabel *)lfirst(lc);
                    char        labelkind;
                    RangeVar    *rel;

                    if(label->labelType == ELabelType){
                        labelkind = LABEL_KIND_EDGE;
                        List        *col_list = NULL;
                        CreateStmt  *c_edgelabelstmt = makeNode(CreateStmt);
                        ColumnDef   *startid = makeNode(ColumnDef);
                        ColumnDef   *startlabelid = makeNode(ColumnDef);
                        ColumnDef   *properties = makeNode(ColumnDef);
                        ColumnDef   *endid = makeNode(ColumnDef);
                        ColumnDef   *endlabelid = makeNode(ColumnDef);


                        startid->colname = "startid";
                        startid->typname = makeTypeName("oid");
                        startid->typname->charset = PG_INVALID_ENCODING;
                        startid->kvtype = ATT_KV_UNDEFINED;
                        startid->inhcount = 0;
                        startid->is_local = true;
                        startid->is_not_null = false;
                        startid->is_from_type = false;
                        startid->storage = 0;
                        startid->cmprs_mode = ATT_CMPR_UNDEFINED;
                        startid->raw_default = NULL;
                        startid->update_default = NULL;
                        startid->cooked_default = NULL;
                        startid->collOid = InvalidOid;
                        startid->fdwoptions = NIL;
                        startid->constraints = NIL;
                        startid->collClause = NULL;
                        startid->clientLogicColumnRef = NULL;
                        startid->columnOptions = NIL;

                        startlabelid->colname = "startlabelid";
                        startlabelid->typname = makeTypeName("oid");
                        startlabelid->typname->charset = PG_INVALID_ENCODING;
                        startlabelid->kvtype = ATT_KV_UNDEFINED;
                        startlabelid->inhcount = 0;
                        startlabelid->is_local = true;
                        startlabelid->is_not_null = false;
                        startlabelid->is_from_type = false;
                        startlabelid->storage = 0;
                        startlabelid->cmprs_mode = ATT_CMPR_UNDEFINED;
                        startlabelid->raw_default = NULL;
                        startlabelid->update_default = NULL;
                        startlabelid->cooked_default = NULL;
                        startlabelid->collOid = InvalidOid;
                        startlabelid->fdwoptions = NIL;
                        startlabelid->constraints = NIL;
                        startlabelid->collClause = NULL;
                        startlabelid->clientLogicColumnRef = NULL;
                        startlabelid->columnOptions = NIL;

                        endid->colname = "endid";
                        endid->typname = makeTypeName("oid");
                        endid->typname->charset = PG_INVALID_ENCODING;
                        endid->kvtype = ATT_KV_UNDEFINED;
                        endid->inhcount = 0;
                        endid->is_local = true;
                        endid->is_not_null = false;
                        endid->is_from_type = false;
                        endid->storage = 0;
                        endid->cmprs_mode = ATT_CMPR_UNDEFINED;
                        endid->raw_default = NULL;
                        endid->update_default = NULL;
                        endid->cooked_default = NULL;
                        endid->collOid = InvalidOid;
                        endid->fdwoptions = NIL;
                        endid->constraints = NIL;
                        endid->collClause = NULL;
                        endid->clientLogicColumnRef = NULL;
                        endid->columnOptions = NIL;

                        endlabelid->colname = "endlabelid";
                        endlabelid->typname = makeTypeName("oid");
                        endlabelid->typname->charset = PG_INVALID_ENCODING;
                        endlabelid->kvtype = ATT_KV_UNDEFINED;
                        endlabelid->inhcount = 0;
                        endlabelid->is_local = true;
                        endlabelid->is_not_null = false;
                        endlabelid->is_from_type = false;
                        endlabelid->storage = 0;
                        endlabelid->cmprs_mode = ATT_CMPR_UNDEFINED;
                        endlabelid->raw_default = NULL;
                        endlabelid->update_default = NULL;
                        endlabelid->cooked_default = NULL;
                        endlabelid->collOid = InvalidOid;
                        endlabelid->fdwoptions = NIL;
                        endlabelid->constraints = NIL;
                        endlabelid->collClause = NULL;
                        endlabelid->clientLogicColumnRef = NULL;
                        endlabelid->columnOptions = NIL;

                        properties->colname = "properties";
                        properties->typname = makeTypeName("jsonb");
                        properties->typname->charset = PG_INVALID_ENCODING;
                        properties->kvtype = ATT_KV_UNDEFINED;
                        properties->inhcount = 0;
                        properties->is_local = true;
                        properties->is_not_null = false;
                        properties->is_from_type = false;
                        properties->storage = 0;
                        properties->cmprs_mode = ATT_CMPR_UNDEFINED;
                        properties->raw_default = NULL;
                        properties->update_default = NULL;
                        properties->cooked_default = NULL;
                        properties->collOid = InvalidOid;
                        properties->fdwoptions = NIL;
                        properties->constraints = NIL;
                        properties->collClause = NULL;
                        properties->clientLogicColumnRef = NULL;
                        properties->columnOptions = NIL;

                        col_list = list_make4(startid, startlabelid, endid, endlabelid);
                        col_list = list_concat(col_list, list_make1(properties));

                        rel = makeRangeVar(NULL, label->labelName->val.str, -1);

                        c_edgelabelstmt->relation = rel;
                        c_edgelabelstmt->tableElts = col_list;
                        c_edgelabelstmt->inhRelations = NIL;
                        c_edgelabelstmt->constraints = NIL;
                        c_edgelabelstmt->options = NIL;
                        c_edgelabelstmt->oncommit = ONCOMMIT_NOOP;
                        c_edgelabelstmt->row_compress = REL_CMPRS_PAGE_PLAIN;
                        c_edgelabelstmt->tablespacename = NULL;
                        c_edgelabelstmt->if_not_exists = true;
#ifdef PGXC
                        c_edgelabelstmt->distributeby = NULL;
                        c_edgelabelstmt->subcluster = NULL;
#endif
                        c_edgelabelstmt->tableOptions = NIL;
                        c_edgelabelstmt->partTableState = NULL;
                        c_edgelabelstmt->internalData = NULL;
                        c_edgelabelstmt->autoIncStart = NULL;
                        c_edgelabelstmt->charset = PG_INVALID_ENCODING;

#ifdef PGXC
                        CreateCommand(c_edgelabelstmt, query_string, params, is_top_level, sent_to_remote, isCTAS);
#else
                        CreateCommand(c_edgelabelstmt, query_string, params, is_top_level, isCTAS);
#endif

                    }else if(label->labelType == VLabelType){
                        labelkind = LABEL_KIND_VERTEX;
                        List        *col_list = NULL;
                        CreateStmt  *c_vertexlabelstmt = makeNode(CreateStmt);
                        ColumnDef   *vid = makeNode(ColumnDef);
                        ColumnDef   *properties = makeNode(ColumnDef);

                        vid->colname = "id";
                        vid->typname = makeTypeName("serial");
                        vid->typname->charset = PG_INVALID_ENCODING;
                        vid->kvtype = ATT_KV_UNDEFINED;
                        vid->inhcount = 0;
                        vid->is_local = true;
                        vid->is_not_null = false;
                        vid->is_from_type = false;
                        vid->storage = 0;
                        vid->cmprs_mode = ATT_CMPR_UNDEFINED;
                        vid->raw_default = NULL;
                        vid->update_default = NULL;
                        vid->cooked_default = NULL;
                        vid->collOid = InvalidOid;
                        vid->fdwoptions = NIL;
                        vid->constraints = NIL;
                        vid->collClause = NULL;
                        vid->clientLogicColumnRef = NULL;
                        vid->columnOptions = NIL;
                        ...
                        col_list = list_make2(vid, properties);

                        rel = makeRangeVar(NULL, label->labelName->val.str, -1);

                        c_vertexlabelstmt->relation = rel;
                        c_vertexlabelstmt->tableElts = col_list;
                        c_vertexlabelstmt->inhRelations = NIL;
                        c_vertexlabelstmt->constraints = NIL;
                        c_vertexlabelstmt->options = NIL;
                        c_vertexlabelstmt->oncommit = ONCOMMIT_NOOP;
                        c_vertexlabelstmt->row_compress = REL_CMPRS_PAGE_PLAIN;
                        c_vertexlabelstmt->tablespacename = NULL;
                        c_vertexlabelstmt->if_not_exists = true;
#ifdef PGXC
                        c_vertexlabelstmt->distributeby = NULL;
                        c_vertexlabelstmt->subcluster = NULL;
#endif
                        c_vertexlabelstmt->tableOptions = NIL;
                        c_vertexlabelstmt->partTableState = NULL;
                        c_vertexlabelstmt->internalData = NULL;
                        c_vertexlabelstmt->autoIncStart = NULL;
                        c_vertexlabelstmt->charset = PG_INVALID_ENCODING;

#ifdef PGXC
                        CreateCommand(c_vertexlabelstmt, query_string, params, is_top_level, sent_to_remote, isCTAS);
#else
                        CreateCommand(c_vertexlabelstmt, query_string, params, is_top_level, isCTAS);
#endif
                    }else{
                        ereport(ERROR,
                            (errcode(ERRCODE_OPERATE_NOT_SUPPORTED),
                                errmsg("The labelType is not recognised.")));
                        return;
                    }
                    /*
                    * Insert meta-data for the newly created label object.
                    */
                    Oid         label_oid = InvalidOid;
                    bool        label_nulls[Natts_mm_label];
                    Datum       label_values[Natts_mm_label];
                    HeapTuple   tup;

                    Relation mm_label = relation_open(LabelRelationId, RowExclusiveLock);
                    memset(label_values, 0, sizeof(label_values));
                    memset(label_nulls, false, sizeof(label_nulls));

                    label_oid = RangeVarGetRelid(rel, AccessShareLock, false);
                    Relation new_label = RelationIdGetRelation(label_oid);

                    label_values[Anum_mm_label_labelname - 1] = DirectFunctionCall1(namein, CStringGetDatum(rel->relname));
                    label_values[Anum_mm_label_labelid - 1] = ObjectIdGetDatum(label_oid);
                    label_values[Anum_mm_label_graphid - 1] = ObjectIdGetDatum(graph_oid);
                    label_values[Anum_mm_label_labkind - 1] = CharGetDatum(labelkind);

                    RelationClose(new_label);

                    tup = heap_form_tuple(RelationGetDescr(mm_label), label_values, label_nulls);

                    tup->t_tableOid = label_oid;

                    CatalogTupleInsert(mm_label, tup);

                    heap_freetuple(tup);

                    relation_close(mm_label, RowExclusiveLock);
                }
            ...
            case T_AlterGraphLabelStmt: {
                //TODO
            } break;

            case T_CreateVectorsStmt: {
                CreateVectorsStmt   *vec_stmt = (CreateVectorsStmt*)parse_tree;
                CreateStmt          *c_stmt = makeNode(CreateStmt);
                ColumnDef           *col_id = makeNode(ColumnDef);
                ColumnDef           *col_vec = makeNode(ColumnDef);
                List                *templist = NULL;
                ListCell            *lc = NULL;
                Relation            mm_vec;
                bool		        nulls[Natts_mm_vector];
                Datum		        values[Natts_mm_vector];
                HeapTuple	        tup;
                Oid                 vec_oid = InvalidOid;
                Relation            new_vec;

                c_stmt->relation = vec_stmt->relation;
                c_stmt->tableElts = vec_stmt->tableElts;
                c_stmt->inhRelations = NIL;
                c_stmt->constraints = NIL;
                c_stmt->options = NIL;
                c_stmt->oncommit = ONCOMMIT_NOOP;
                ...
                c_stmt->charset = PG_INVALID_ENCODING;
                /*
                 * Transform the CreateVectorStmt to a CreateStmt:
                 * append an ID column and a Vector column to column list 
                 * in the Create Stmt.
                 */
                foreach(lc, c_stmt->tableElts)
                {
                    ...
                    if ((pg_strcasecmp(tempcoldef->colname, "ID") == 0) && (pg_strcasecmp(tempcoldef->colname, FIXED_VECTOR_COLNAME) == 0))
                        elog(ERROR, "The name of descriptive attributes in Vector can not be 'id' or 'vec'.");
                }
                col_id->colname = "id";
                col_id->typname = makeTypeName("serial");
                col_id->typname->charset = PG_INVALID_ENCODING;
                ...
                A_Const *const_n = makeNode(A_Const);

                const_n->val.type = T_Integer;
                const_n->val.val.ival = vec_stmt->dim;
                const_n->location = -1;

                col_vec->colname = FIXED_VECTOR_COLNAME;
                col_vec->typname = makeTypeName("vector");
                col_vec->typname->typmods = list_make1(const_n);
                col_vec->typname->charset = PG_INVALID_ENCODING;
				col_vec->kvtype = ATT_KV_UNDEFINED;
                col_vec->inhcount = 0;
                col_vec->is_local = true;
                col_vec->is_not_null = false;
                col_vec->is_from_type = false;
                col_vec->storage = 0;
                col_vec->cmprs_mode = ATT_CMPR_UNDEFINED;
                col_vec->raw_default = NULL;
                col_vec->update_default = NULL;
                col_vec->cooked_default = NULL;
                col_vec->collOid = InvalidOid;
                col_vec->fdwoptions = NIL;
                col_vec->constraints = NIL;
                col_vec->collClause = NULL;
                col_vec->clientLogicColumnRef = NULL;
				col_vec->columnOptions = NIL;

                templist = list_make1(col_vec);
                c_stmt->tableElts = list_concat(c_stmt->tableElts, templist);
                 ...
                 * Insert meta-data for the newly created documents object.
                 */
                mm_vec = relation_open(VectorRelationId, RowExclusiveLock);
                memset(values, 0, sizeof(values));
                memset(nulls, false, sizeof(nulls));
                
                vec_oid = RangeVarGetRelid(c_stmt->relation, AccessShareLock, false);
                new_vec = RelationIdGetRelation(vec_oid);

                values[Anum_mm_vector_vecname - 1] =
                    DirectFunctionCall1(namein, CStringGetDatum(c_stmt->relation->relname));
                values[Anum_mm_vector_vecid - 1] = ObjectIdGetDatum(vec_oid);
                values[Anum_mm_vector_vecnsp - 1] = ObjectIdGetDatum(RelationGetNamespace(new_vec));
                values[Anum_mm_vector_dims - 1] = Int32GetDatum(vec_stmt->dim);

                RelationClose(new_vec);

                tup = heap_form_tuple(RelationGetDescr(mm_vec), values, nulls);

                tup->t_tableOid = vec_oid;

                CatalogTupleInsert(mm_vec, tup);
                heap_freetuple(tup);

                relation_close(mm_vec, RowExclusiveLock);
