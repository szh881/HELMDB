...
ArrayScanPath* create_array_scan_path(PlannerInfo* root, RelOptInfo* rel, Relids required_outer, int dop){
    ArrayScanPath* pathnode = makeNode(ArrayScanPath);

    // create seqscan path firstly
    Path* seqscan_path = create_seqscan_path(root, rel, required_outer, dop);   

    // fill in the ArrayScanPath information
    pathnode->path = *seqscan_path;
    pathnode->path.type = T_ArrayScanPath;
    pathnode->path.pathtype = T_ArrayScan;

    pfree(seqscan_path);
    return pathnode;
}

DocumentScanPath* create_document_scan_path(PlannerInfo* root, RelOptInfo* rel, Relids required_outer, int dop){
    DocumentScanPath* pathnode = makeNode(DocumentScanPath);
    // create seqscan path firstly
    Path* seqscan_path = create_seqscan_path(root, rel, required_outer, dop);   

    // fill in the DocumentScanPath information
    pathnode->path = *seqscan_path;
    pathnode->path.type = T_DocumentScanPath;
    pathnode->path.pathtype = T_DocumentScan;

    pfree(seqscan_path);
    return pathnode;
}

VectorScanPath* create_vector_scan_path(PlannerInfo* root, RelOptInfo* rel, Relids required_outer, int dop){
    VectorScanPath* pathnode = makeNode(VectorScanPath);
    // create seqscan path firstly
    Path* seqscan_path = create_seqscan_path(root, rel, required_outer, dop);   

    // fill in the VectorScanPath information 
    pathnode->path = *seqscan_path;
    pathnode->path.type = T_VectorScanPath;
    pathnode->path.pathtype = T_VectorScan;

    pfree(seqscan_path);
    return pathnode;
}

GraphScanPath* create_graph_scan_path(PlannerInfo* root, RelOptInfo* rel, Relids required_outer, int dop){
    GraphScanPath* pathnode = makeNode(GraphScanPath);

    // create seqscan path firstly
    Path* seqscan_path = create_seqscan_path(root, rel, required_outer, dop);   

    // fill in the GraphScanPath information
    pathnode->path = *seqscan_path;
    pathnode->path.type = T_GraphScanPath;
    pathnode->path.pathtype = T_GraphScan;
    
    // pass MATCH info to GraphScanPath
    pathnode->cypher_rels = rel->cypher_rels;
    pathnode->cypher_restrictexprlist = rel->cypher_restrictexprlist;
