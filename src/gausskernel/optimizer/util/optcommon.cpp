    ...
    switch (nodeTag(plan)) {
        ...
        case T_IndexScan:
            *pt_operation = "INDEX";
            ...
            if(((IndexScan*)plan)->scan.mm_type != NORMAL_TABLE_MODEL_TYPE){
                StringInfo si = makeStringInfo();

                switch (((Scan*)plan)->mm_type) {
                    case ARRAY_TABLE_MODEL_TYPE:{ // array table
                        appendStringInfo(si, "Array %s", *pname);
                    }break;
                    case DOCUMENT_TABLE_MODEL_TYPE:{ // document table
                        appendStringInfo(si, "Document %s", *pname);
                    }break;
                    case VECTOR_TABLE_MODEL_TYPE:{ // vector table
                        appendStringInfo(si, "Vector %s", *pname);
                    }break;
                    case GRAPH_TABLE_MODEL_TYPE:{ // graph table
                        appendStringInfo(si, "Graph %s", *pname);
                    }break;
                    default:{
                        appendStringInfo(si, "%s", *pname);
                    }
                }
                *pname = si->data;
            }
        ...
        case T_LockRows:
            *pname = *sname = *pt_operation = "LockRows";
            break;
        case T_Limit:
            // if child node is  vector index scan ,then limit node is topk  
            if(IsA(plan->lefttree, IndexScan) && ((IndexScan*)plan->lefttree)->scan.mm_type == VECTOR_TABLE_MODEL_TYPE){
                *pname = *sname = *pt_operation = "Topk";
            }else{
        ...
        case T_ArrayScan: {
            *pname = *sname =  "Array Scan";
        } break;
        case T_DocumentScan: {
            *pname = *sname =  "Document Scan";
        } break;
        case T_VectorScan: {
            *pname = *sname =  "Vector Scan";
        } break;
        case T_GraphScan: {
