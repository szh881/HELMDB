...
static void set_plain_rel_pathlist(PlannerInfo* root, RelOptInfo* rel, RangeTblEntry* rte)
{
    ...
    //add path according to mm_type
    if(rel->mm_type != NORMAL_TABLE_MODEL_TYPE){
        bool mm_flag = true;
        switch(rel->mm_type){
            case ARRAY_TABLE_MODEL_TYPE:
                add_path(root, rel, (Path*)create_array_scan_path(root, rel, required_outer, u_sess->opt_cxt.query_dop));
                break;
            case GRAPH_TABLE_MODEL_TYPE:
                //add graph path
                add_path(root, rel, (Path*)create_graph_scan_path(root, rel, rel->lateral_relids, u_sess->opt_cxt.query_dop));
                break;
            case VECTOR_TABLE_MODEL_TYPE:
                add_path(root, rel, (Path*)create_vector_scan_path(root, rel, rel->lateral_relids, u_sess->opt_cxt.query_dop));
                /* Consider index scans */
                create_index_paths(root, rel);
                break;
            case DOCUMENT_TABLE_MODEL_TYPE:
                add_path(root, rel, (Path*)create_document_scan_path(root, rel, rel->lateral_relids, u_sess->opt_cxt.query_dop));
                break;
            default:
                //error
                mm_flag = false;
...
#ifdef ENABLE_MOT
    if (!isrp && rel->fdwroutine->GetFdwType &&
        ((rel->fdwroutine->GetFdwType() == MOT_ORC) || (rel->fdwroutine->GetFdwType() == NVM_ORC))) {
        /* Consider index scans */
        create_index_paths(root, rel);
        if (rel->orientation == REL_ROW_ORIENTED) {
            create_tidscan_paths(root, rel);
...
#ifdef ENABLE_MOT
    if (!(rel->fdwroutine->GetFdwType &&
          ((rel->fdwroutine->GetFdwType() == MOT_ORC) || (rel->fdwroutine->GetFdwType() == NVM_ORC))) || isrp) {
#endif
        set_cheapest(rel);
#ifdef ENABLE_MOT
    }
...
static RelOptInfo* make_rel_from_joinlist(PlannerInfo* root, List* joinlist)
{
    ...
    foreach (jl, joinlist) {
        ...
        // if this is a cypher query, then we neglect the subsequent jlnode
        if(thisrel->mm_type == GRAPH_TABLE_MODEL_TYPE){
            levels_needed = 1;
            break;
        }
    }
