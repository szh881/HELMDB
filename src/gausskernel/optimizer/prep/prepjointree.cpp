...
static bool is_cypher_subquery(RangeTblEntry* rte)
{
    return rte->mm_type == GRAPH_TABLE_MODEL_TYPE;
}
...
static bool is_simple_subquery(Query* subquery, RangeTblEntry *rte, JoinExpr *lowest_outer_join)
{
    ...
    /*
     *  if rte is a graph table, namely subquery is a cypher query, then it's not a simple subquery
     */
    if(is_cypher_subquery(rte)){
        return false;
    }
