...
/* For Cypher Query Explain */
static void show_cypher_match(List *cypher_restrictexprlist, const char *qlabel, PlanState *planstate, List *ancestors, ExplainState *es);
static char* deparse_cypher_node(Node * node);
static char* deparse_cypher_rel(Node * node);
/*
 * ExplainQuery -
    ...
    PlanState* planstate, List* ancestors, const char* relationship, const char* plan_name, ExplainState* es)
{
    ...
    switch (nodeTag(plan)) {
        ...
        case T_GraphScan:
            ...
            show_cypher_match(((GraphScan*)plan)->cypher_restrictexprlist, "Match Pattern", planstate, ancestors, es);
            break;
        case T_CStoreScan:
#ifdef ENABLE_MULTIPLE_NODES
        case T_TsStoreScan:
...
static char* deparse_cypher_node(Node * node){
    if(!IsA(node,CypherNode)){
        return NULL;
    }
    StringInfo si = makeStringInfo();
    CypherNode* cypher_node = (CypherNode*)node;

    appendStringInfo(si, "(");
    // node name
    if(cypher_node->variable != NULL){
        appendStringInfo(si, "%s", ((CypherName*)cypher_node->variable)->name);
    }

    // node label
    if(cypher_node->label != NULL){
        appendStringInfo(si, ":%s", ((CypherName*)cypher_node->label)->name);
    }
    appendStringInfo(si, ")");
...
static char* deparse_cypher_rel(Node * node){
    if(!IsA(node,CypherRel)){
        return NULL;
    }
    StringInfo si = makeStringInfo();
    CypherRel* cypher_rel = (CypherRel*)node;

    // left arrow
    if(cypher_rel->direction == CYPHER_REL_DIR_LEFT){
        appendStringInfo(si, "<-");
    }else{
        appendStringInfo(si, "-");
    }
    ...
    // rel name
    if(cypher_rel->variable != NULL){
        appendStringInfo(si, "%s", ((CypherName*)cypher_rel->variable)->name);
    }

    // rel label
    if(cypher_rel->types != NULL){
        appendStringInfo(si, ":%s", ((CypherName*)linitial(cypher_rel->types))->name);
    }

    // right arrow
    appendStringInfo(si, "]");
    if(cypher_rel->direction == CYPHER_REL_DIR_RIGHT){
        appendStringInfo(si, "->");
    }else{
        appendStringInfo(si, "-");
    }
...
static void show_cypher_match(List *cypher_restrictexprlist, const char *qlabel, PlanState *planstate, List *ancestors, ExplainState *es)
{
    char* exprstr = NULL;
    StringInfo si = makeStringInfo();
    if(cypher_restrictexprlist == NULL || list_length(cypher_restrictexprlist)==0){
        return;
    }
    /* Deparse the match info */
    ListCell* lc = NULL;
    ListCell* lc1 = NULL;
    ListCell* lc2 = NULL;
    foreach(lc, cypher_restrictexprlist){
        CypherMatchRestrictExpr* expr = (CypherMatchRestrictExpr*)lfirst(lc);
        int node_list_len = list_length(expr->match_node_list);
        int rel_list_len = list_length(expr->match_rel_list);
        if(node_list_len == 0 || rel_list_len == 0 || node_list_len != (rel_list_len + 1)){
            return;
        }
        forboth(lc1, expr->match_node_list, lc2, expr->match_rel_list){
            CypherNode* node = (CypherNode*)lfirst(lc1);
            CypherRel* rel = (CypherRel*)lfirst(lc2);
            appendStringInfo(si, "%s", deparse_cypher_node((Node*)node));
            appendStringInfo(si, "%s", deparse_cypher_rel((Node*)rel));
        }
        CypherNode* node =(CypherNode*)llast(expr->match_node_list);
        appendStringInfo(si, "%s", deparse_cypher_node((Node*)node));
    }
