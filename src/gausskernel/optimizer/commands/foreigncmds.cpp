...
void CreateForeignIndex(IndexStmt* stmt, Oid indexRelationId)
{
    ...
    if (rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE &&
        (isMOTFromTblOid(RelationGetRelid(rel)) || (isNVMFromTblOid(RelationGetRelid(rel))))) {
        ...
        if (fdwroutine->ValidateTableDef != NULL) {
            ...
            StorageEngineType storageEngineType = (isMOTFromTblOid(RelationGetRelid(rel)) ? SE_TYPE_MOT : SE_TYPE_NVMDB);
...
#ifdef ENABLE_NVMDB
    if (isNVMTableFromSrvName(stmt->servername)) {
        SetCurrentTransactionStorageEngine(SE_TYPE_NVMDB);
    }
#endif
