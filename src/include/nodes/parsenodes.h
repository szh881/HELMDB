...
{
	NodeTag		type;
	List *match_pattern; // the untransformed graph match pattern given by user
} CypherStmt;
...
typedef struct RangeTblEntry {
    ...
    List *subpartitionOidList;  /*
                                 ...
                                 * or delete from table_name partition (partition_name, ...)
                                 */
    bool isJSON = false; //由json_array_elements()生成的rte会被标记为true，会进行点操作的语法树重写 
    /* For Multi-model Query */
    char mm_type;             /* determines the model of table:
                                 * 't': normal   table
                                 * 'g': graph    table
                                 * 'd': document table
                                 * 'v': vector   table
 ...
 * ----------------------
 */
typedef struct CreateArrayStmt {
    NodeTag type;
    List    *dim_list;
    CreateStmt *stmt;
} CreateArrayStmt;

typedef struct CreateDocumentsStmt {
    NodeTag type;
...
typedef struct CreateGraphStmt {
    ...
    List *labelList;    /* List of label nodes */
    bool if_not_exists; /* just do nothing if it already exists? */

    int charset;
} CreateGraphStmt;

typedef enum LabelType { ELabelType, VLabelType } LabelType;
...
typedef enum LabelOperatorType { AddGraphLabel, DropGraphLabel, ModifyGraphLabel } LabelOperatorType;

typedef struct AlterGraphLabelStmt {
    NodeTag type;
    RangeVar *relation;
    List *labelList;    /* List of label nodes */
    ObjectType relkind; /* type of object */
    LabelOperatorType labelOperatorType;
    bool missing_ok;       /* skip error if table missing */
    bool fromCreate;       /* from create stmt */
    bool need_rewrite_sql; /* after rewrite rule, need to rewrite query string */
} AlterGraphLabelStmt;

typedef struct CreateVectorsStmt {
    NodeTag type;
    RangeVar *relation; /* relation to create */
    List *tableElts;    /* column definitions (list of ColumnDef) */
...
typedef struct ArraySelectClause{
    NodeTag     type;
