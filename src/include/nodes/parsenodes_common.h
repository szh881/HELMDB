...
#define OBJECT_IS_SEQUENCE(obj) ((obj) == OBJECT_LARGE_SEQUENCE || (obj) == OBJECT_SEQUENCE)
...
//default: Get all the elements in a jsonArray
typedef struct Doc_Indices {
    NodeTag type;
    Node *returnTarget;       
    Node *filterClause;      /* WHERE qualification */
    Node *limitClause;   // DocLimit
} Doc_Indices;

typedef struct DocLimit{
  NodeTag type;
...
typedef struct InsertDocumentStmt {
    NodeTag type;
    RangeVar *relation;         /* relation to insert into */
    List *values;          //   list of A_JSONObjectExpr     
} InsertDocumentStmt;
...
typedef struct DeleteDocumentStmt {
    NodeTag type;
    RangeVar *relation;     
    Node *whereClause;      /* qualifications */
    Node *limitClause; /* limit row count */
...
typedef struct UpdateDocumentStmt {
    NodeTag type;
    RangeVar *relation;    
    List *targetList;       /* the target list (of ResTarget) */
    Node *whereClause;      /* qualifications */
...
typedef enum class ModelType{
  relation = 0,document,array,graph
} ModelType;
...
typedef struct SelectStmt {
    ...
    WithClause *withClause; /* WITH clause */
    ModelType resultModel = ModelType::relation;
    ...
    bool is_array_select;       // 是否是数组选择操作?
    Node * arraySelectClause;   // 数组选择操作的信息结构体(包括维度、属性相关信息)
} SelectStmt;
...
/*
 * RangeCypher - Graph match query appearing in a From clause  
 */
typedef struct RangeCypher{
    NodeTag type;
    Node *relation; // graph relation to execute query
    Node *cypher_query; /* the untransformed sub-select clause */
    Alias *alias;   /* table alias & optional column aliases */
} RangeCypher;
...
typedef struct A_JSONArrayExpr {
    NodeTag type;
    List *elements; /* array element expressions */
    int location;   /* token location, or -1 if unknown */
} A_JSONArrayExpr;

typedef struct A_JSONObjectExpr {
    NodeTag type;
    List *elements; /* JSONObjectPair element expressions */
    int location;   /* token location, or -1 if unknown */
} A_JSONObjectExpr;
...
typedef struct Query {
    ...
    /* For Graph Query */
    bool is_cypher_query;           
    CypherMatchExpr * cypher_match;
    // For Array Query
    bool is_array_select;           //是否为数组选择操作
    Node * arraySelectClause;       //数组操作
} Query;
